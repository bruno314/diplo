        ...
        try {
            ...
            function process(options) {
                ...
                $.each(arr, function (i, j) {
                    ...
                    html = el.html();
                    if (matches != null)$.each(matches, function (index, value) {
                        ...
                        if (c.length == 4) {
                            ...
                            for (ci = 2; ci < 4; ci++)if (c[ci].substr(-1) + c[ci].charAt(0) != "''")c[ci] = j[c[ci]]; else c[ci] = c[ci].substr(1, c[ci].length - 2);
                            console.log(c[0] + condition + c[1]);
                            if (eval(c[0] + condition + c[1]))html = html.replace(value, unescape(c[2])); else html = html.replace(value, unescape(c[3]))
                        } else {
                            t = t.split("||");
                            newval = json_value(t[0], j);
                            if ((newval == "" || newval == undefined) && t.length != 1)html = html.replace(value, t[1]); else html = html.replace(value, json_value(t[0], j))
                        }
                    });
                    html = html.replace("jsrc", "src");
                    if (options.prepend == false)main.append(html); else main.prepend(html);
                    if (jrepeat.length > 0) {
                        jrepeat.each(function (e, t) {
